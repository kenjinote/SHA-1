...
DWORD WINAPI ThreadFunc(LPVOID p)
{
	...
	if (!CryptCreateHash(hProv, CALG_SHA1, 0, 0, &hHash)){ lstrcpy(pData->szHashValue, TEXT("ハッシュオブジェクトのハンドルの取得に失敗しました。")); goto END2; }
	TCHAR buff[64 * 1024];
	DWORD wReadSize;
	...
	else if (bRet)
	{
		...
		wsprintf(pData->szHashValue, TEXT("%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X"), hash[0], hash[1], hash[2], hash[3], hash[4], hash[5], hash[6], hash[7], hash[8], hash[9], hash[10], hash[11], hash[12], hash[13], hash[14], hash[15], hash[16], hash[17], hash[18], hash[19]);
		wParam = 1;
	}
	else
	{
...
VOID CalcSha1(HWND hList, LPCTSTR lpszFilePath)
{
